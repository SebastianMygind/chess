...
const SIMPLE_MOVE_INDEX: usize = 0;
...
fn get_simple_move(
    ...
    if let Some(position_opt) = pawn_moves[SIMPLE_MOVE_INDEX].get_new_position(position) {
...
fn get_simple_capture(
    ...
    for attack in pawn_moves[ATTACK_START_INDEX..].into_iter() {
        let new_position: usize;
...
fn get_simple_capture_promotion(
    ...
    if let Some(capture_move) = get_simple_capture(position, chessboard, pawn_moves) {
        let mut promotions: Vec<LegalMove> = Vec::with_capacity(4);
...
pub fn get_pawn_moves(position: usize, chessboard: &ChessBoard) -> Vec<LegalMove> {
    ...
    if current_rank == promotion_rank {
        ...
        if let Some(mut promotions) =
            get_simple_capture_promotion(position, chessboard, pawn_moves, promotions)
        {
            moves.append(&mut promotions);
        }
    } else {
    ...
    if double_pawn_rank == current_rank {
        if let Some(double_pawn_move) = get_double_move(position, chessboard, pawn_moves) {
